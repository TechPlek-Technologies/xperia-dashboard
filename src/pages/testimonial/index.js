...
function ReactTable({ columns, data, renderRowSubComponent, handleAdd }) {
  ...
  const sortBy = { id: 'designation', desc: false };
...
const TestimonialPage = () => {
  ...
  const handleAdd = () => {
    navigate('/add-Testimonial', { replace: true });
  };

  const handleUpdate = (data) => {
    if (data && data.id) {
      navigate(`/update-testimonial/${data.id}`, { replace: true });
    } else {
      console.error('Error: testimonial or testimonial.id is undefined');
    }
  };
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            <Stack direction="row" spacing={1.5} alignItems="center">
              <Stack spacing={0}>
                <Typography variant="subtitle1">{values.designation}</Typography>
              </Stack>
            </Stack>
          );
        }
      },
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            <Stack direction="row" spacing={1.5} alignItems="center">
              <Stack spacing={0}>
                <Typography variant="subtitle1">{values.shortDescription}</Typography>
              </Stack>
            </Stack>
          );
        }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const newData = await getData(`${process.env.REACT_APP_API_URL}/testimonials/all-testimonial`);
        console.log(newData);
